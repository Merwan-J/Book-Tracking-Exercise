...
def varint_from_protobuf_stream(payload: bytes) -> Tuple[int, bytes]:
    ...
    for item in payload:
        char = item if isinstance(item, int) else ord(
...
def decimal_from_protobuf(payload: bytes) -> decimal.Decimal:
    digits = []
    sign = None
    scale = payload[0] if isinstance(payload[0], int) else ord(
        payload[0])  # type: ignore[arg-type]
    payload = payload[1:]
...
class Row:
    ...
    def __getitem__(self, index: Union[int, str]) -> Any:
        ...
        .. versionchanged:: 8.0.12
        """
        int_index = self._resultset.index_of(
            index) if isinstance(index, str) else index
