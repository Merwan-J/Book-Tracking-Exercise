        ...
        if ssl_cert:
            try:
                ...
                raise InterfaceError(
                    f"Invalid Certificate/Key: {err}") from err
...
class Connection:
    ...
    def __init__(self, settings: Dict[str, Any]) -> None:
        ...
        self.router_manager: RouterManager = RouterManager(
            self._routers, settings)
        ...
        self._server_disconnected_reason: Optional[Union[str,
                                                         Tuple[str, int]]] = None
    ...
    def _set_tls_capabilities(self, caps: Dict[str, Any]) -> None:
        ...
        if platform.system() == "Linux":
            ...
            try:
                is_ol7 = "Oracle Linux" in distname and version.split(".")[
                    0] == "7"
            except IndexError:
                is_ol7 = False
    ...
    def add_connection(self, cnx: Optional[PooledConnection] = None) -> None:
        ...
        if not cnx:
            ...
            if tuple(int(n) for n in ver.split("-")[0].split(".")) > (
                ...
                cnx.sql(
                    f"set mysqlx_wait_timeout = {self.max_idle_time}").execute()
    ...
    def get_connection(self, settings: Dict[str, Any]) -> PooledConnection:
        ...
        def set_mysqlx_wait_timeout(cnx: PooledConnection) -> None:
            ...
            if tuple(int(n) for n in ver.split("-")[0].split(".")) > (
                ...
                cnx.sql(
                    f"set mysqlx_wait_timeout = {pool.max_idle_time}").execute()
                ...
                if pool.qsize() > 0:
                    ...
                    with lock:
                        try:
                            ...
                            raise PoolError(
                                "pool max size has been reached") from None
...
class Client:
    ...
    def _set_max_idle_time(self, max_idle_time: int) -> None:
        ...
        self.settings["max_idle_time"] = (
            _CNX_POOL_MAX_IDLE_TIME if max_idle_time == 0 else int(
                max_idle_time / 1000)
        )
    ...
    def _set_queue_timeout(self, queue_timeout: int) -> None:
        ...
        self.settings["queue_timeout"] = (
            _CNX_POOL_QUEUE_TIMEOUT if queue_timeout == 0 else int(
                queue_timeout / 1000)
        )
...
def _validate_settings(settings: Dict[str, Any]) -> None:
    ...
    if "ssl-mode" in settings:
        ...
        try:
            ...
            raise InterfaceError(
                f"Invalid SSL Mode '{settings['ssl-mode']}'") from err
...
def _validate_tls_versions(settings: Dict[str, Any]) -> None:
    ...
    if use_tls_versions:
        ...
        raise InterfaceError(TLS_VERSION_ERROR.format(
            tls_ver, SUPPORTED_TLS_VERSIONS))
