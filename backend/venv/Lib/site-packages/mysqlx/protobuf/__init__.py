...
try:
    ...
    SERVER_MESSAGES = {_MESSAGES[key]: val for key,
                       val in _SERVER_MESSAGES_TUPLES}
    HAVE_PROTOBUF = True
    ...
    if not HAVE_MYSQLXPB_CEXT:
        raise ImportError(
            f"Protobuf is not available: {HAVE_PROTOBUF_ERROR}") from err
...
class Message:
    ...
    def __init__(self, msg_type_name: Optional[str] = None, **kwargs: Any) -> None:
        ...
        self.__dict__["_msg"] = (
            Protobuf.mysqlxpb.new_message(
                msg_type_name) if msg_type_name else None
        )
    ...
    def __getattr__(self, name: str) -> Any:
        try:
            return (
                self._msg[name] if not Protobuf.use_pure else getattr(
                    self._msg, name)
            )
        except KeyError:
    ...
    def from_message(cls, msg_type_name: str, payload: bytes) -> MessageType:
        ...
        """
        msg = cls()
        msg.set_message(Protobuf.mysqlxpb.parse_message(
