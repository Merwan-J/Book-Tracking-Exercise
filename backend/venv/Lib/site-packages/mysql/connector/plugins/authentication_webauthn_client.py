...
class MySQLWebAuthnAuthPlugin(MySQLAuthPlugin):
    ...
    def get_assertion_response(
        ...
        for i in range(number_of_assertions):
            ...
            authenticator_data = cbor_dump_bytes(
    ...
    def auth_response(self, auth_data: bytes, **kwargs: Any) -> Optional[bytes]:
        ...
        try:
            ...
            logger.debug("WebAuthn - relying party id: %s",
                         self.options["rpId"])
        ...
        if not self.client.info.options.get("rk"):
            logger.debug(
                "WebAuthn - Authenticator doesn't support resident keys")
            return b"1"

        logger.debug(
            "WebAuthn - Authenticator with support for resident key found")
        return b"2"
    ...
    def auth_more_response(
        ...
        response = self.get_assertion_response(
            credential_id)  # type: ignore[arg-type]

        logger.debug("WebAuthn - request: %s size: %s",
