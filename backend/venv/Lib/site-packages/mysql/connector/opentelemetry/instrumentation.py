    ...
    * NET_SOCK_FAMILY
    ...
    if is_tcp:
        attrs[SpanAttributes.NET_PEER_NAME] = (
            cnx._host if cnx else cnx_kwargs.get(
                "host", DEFAULT_CONFIGURATION["host"])
        )
        attrs[SpanAttributes.NET_PEER_PORT] = (
            cnx._port if cnx else cnx_kwargs.get(
                "port", DEFAULT_CONFIGURATION["port"])
        )
...
def _instrument_connect(
    connect: Callable[
        ..., Union["MySQLConnection",
                   "CMySQLConnection", "PooledMySQLConnection"]
    ],
    ...
    def wrapper(
        *args: Any, **kwargs: Any
        ...
        if any(key in kwargs for key in CNX_POOL_ARGS):
            logger.warning(
                "Instrumentation for pooled connections not supported")
        ...
        with trace.use_span(kwargs[OPTION_CNX_SPAN], end_on_exit=False) as cnx_span:
            ...
            set_connection_span_attrs(cnx, cnx_span, kwargs)
...
class MySQLInstrumentor:
    ...
    def instrument(self, **kwargs: Any) -> None:
        ...
        if connector.connect != getattr(self, "_original_connect"):
            logger.warning(
                "MySQL Connector/Python module already instrumented.")
            return
    ...
    def instrument_connection(
        ...
        return TracedMySQLConnection(wrapped=connection)
    ...
    def uninstrument(self, **kwargs: Any) -> None:
        ...
        if connector.connect == getattr(self, "_original_connect"):
            logger.warning(
                "MySQL Connector/Python module already uninstrumented.")
            return
