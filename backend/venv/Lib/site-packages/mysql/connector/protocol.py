...
class MySQLProtocol:
    ...
    def make_conn_attrs(conn_attrs: ConnAttrsType) -> bytes:
        ...
        for attr_name in conn_attrs:
            ...
            conn_attrs_packet += conn_attrs[attr_name].encode()
    ...
    def auth_plugin_first_response(
        ...
        try:
            ...
            auth_response = auth_strategy.auth_response(
                auth_data, **plugin_config)
    ...
    def make_auth(
        ...
        if handshake.get("auth_data") is None:
            raise ProgrammingError(
                "Handshake misses authentication info") from None
        ...
        # database name
        response_payload += MySQLProtocol.connect_with_db(
            client_flags, database)
        ...
        for pair in pairs:
            ...
            try:
                ...
                try:
                    ...
                    raise InterfaceError(
                        f"{errmsg} ({lbl}:{repr(val)})") from err
        return res
    ...
    def _parse_binary_values(
        ...
        for pos, field in enumerate(fields):
            ...
            if field[1] in (
                ...
                (packet, value) = self._parse_binary_timestamp(
                    packet, field[1])
                values.append(value)
    ...
    def prepare_stmt_send_long_data(
        ...
        Returns a string.
        """
        packet: bytearray = utils.int4store(
            statement) + utils.int2store(param) + data
        return packet
