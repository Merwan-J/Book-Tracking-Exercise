...
MYSQL_CNX_CLASS: Union[type, Tuple[type, ...]] = (
    MySQLConnection if CMySQLConnection is None else (
...
def _get_pooled_connection(**kwargs: Any) -> PooledMySQLConnection:
    ...
    pool_name = (
        kwargs["pool_name"] if "pool_name" in kwargs else generate_pool_name(
            **kwargs)
    )
    ...
    def set_config(self, **kwargs: Any) -> None:
        ...
        with CONNECTION_POOL_LOCK:
            try:
                ...
                raise PoolError(
                    f"Connection configuration not valid: {err}") from err
    ...
    def get_connection(self) -> PooledMySQLConnection:
        ...
        with CONNECTION_POOL_LOCK:
            try:
                ...
                raise PoolError(
                    "Failed getting connection; pool exhausted") from err
