...
class MySQLConnection(MySQLConnectionAbstract):
    ...
    def _do_auth(
        ...
        client_flags: int = 0,
        charset: int = 45,
        ssl_options: Optional[Dict[str,
                                   Optional[Union[str, bool, List[str]]]]] = None,
        ...
        if send_empty_packet:
            try:
                ...
                raise OperationalError(
                    "MySQL Connection not available") from err
    ...
    def _handle_load_data_infile(self, filename: str) -> OkPacketType:
        ...
        try:
            data_file = open(
                file_name, "rb")  # pylint: disable=consider-using-with
            ...
            try:
                ...
                raise OperationalError(
                    "MySQL Connection not available") from err
            raise InterfaceError(
                f"File '{file_name}' could not be read") from None
        finally:
            try:
                data_file.close()
    ...
    def cmd_change_user(
        ...
        if charset < 0:
            raise ValueError(
                "charset should be either zero or a postive integer")
    ...
    def ping(self, reconnect: bool = False, attempts: int = 1, delay: int = 0) -> None:
        ...
        try:
            ...
            if reconnect:
                ...
                raise InterfaceError(
                    "Connection to MySQL is not available") from err
    ...
    def cmd_stmt_reset(self, statement_id: int) -> None:
        ...
        Returns a dict()
        """
        self._handle_ok(self._send_cmd(
            ServerCmd.STMT_RESET, int4store(statement_id)))
