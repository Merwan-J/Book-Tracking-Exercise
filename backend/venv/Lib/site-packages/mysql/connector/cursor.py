...
class MySQLCursor(CursorBase):
    ...
    def _handle_noresultset(self, res: ResultType) -> None:
        ...
        try:
            ...
            raise ProgrammingError(
                f"Failed handling non-resultset; {err}") from None
    ...
    def _batch_insert(
        ...
        try:
            ...
            raise InterfaceError(
                f"Failed executing the operation; {err}") from None
    ...
    def executemany(
        ...
        try:
            ...
            raise ProgrammingError(
                "Parameters for query must be an Iterable") from err
    ...
    def callproc(
        ...
        try:
            ...
            raise InterfaceError(
                f"Failed calling stored routine; {err}") from None
    ...
    def __str__(self) -> str:
        ...
        if self._executed:
            try:
                executed = self._executed.decode(
                    "utf-8")  # type: ignore[union-attr]
            except AttributeError:
...
class MySQLCursorPrepared(MySQLCursor):
    ...
    def __init__(self, connection: Optional[Type[MySQLConnectionAbstract]] = None):
        ...
        self._rows: Optional[List[RowType]] = None
        self._next_row: int = 0
        self._prepared: Optional[Dict[str,
                                      Union[int, List[DescriptionType]]]] = None
    ...
    def execute(
        ...
        if isinstance(params, dict):
            replacement_keys = re.findall(
                RE_SQL_PYTHON_CAPTURE_PARAM_NAME, operation)
            try:
...
class MySQLCursorDict(MySQLCursor):
    ...
    def _row_to_python(
        self,
        rowdata: RowType,
        desc: Optional[List[DescriptionType]
