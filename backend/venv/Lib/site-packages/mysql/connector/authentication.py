...
class MySQLAuthenticator:
    ...
    def _mfa_n_factor(
        ...
        while pkt[4] == MFA_STATUS:
            ...
            new_strategy_name, auth_data = MySQLProtocol.parse_auth_next_factor(
                pkt)
            self._switch_auth_strategy(
                new_strategy_name, password_factor=n_factor)
            logger.debug("MFA %i factor %s", n_factor,
                         self._auth_strategy.name)
    ...
    def _handle_server_response(
        ...
        if pkt[4] == AUTH_SWITCH_STATUS:
            ...
            new_strategy_name, auth_data = MySQLProtocol.parse_auth_switch_request(
                pkt)
