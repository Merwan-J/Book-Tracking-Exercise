...
AUTHORITY_REGEX = re.compile(
    (
        ...
        # Either any character sequence not including ':',
        host="(\\[.*\\]|[^:]*)",
...
def urlparse(url: str = "", **kwargs: typing.Optional[str]) -> ParseResult:
    ...
    parsed_fragment: typing.Optional[str] = (
        None if fragment is None else quote(
            fragment, safe=SUB_DELIMS + ":/?#[]@")
    )
...
def quote(string: str, safe: str = "/") -> str:
    ...
    NON_ESCAPED_CHARS = UNRESERVED_CHARACTERS + safe
    return "".join(
        [char if char in NON_ESCAPED_CHARS else percent_encode(
